...
> = P extends `${infer Key}.${infer Rest}`
  ? Key extends keyof T
    ? ConsiderOptional<Pick<T, Key>, PathObject<T[Key], Rest>>
    : never
  : P extends keyof T
  ? undefined extends T[P]
    ? { [p in P]?: undefined }
    : ConsiderOptional<Pick<T, P>, T[P]>
  : never;

export declare type NamespaceAccessors = Path<OnepercentUtility>;
export declare type FromOnePercentUtility<P extends NamespaceAccessors> =
  keyof any extends import("type-fest").Get<OnepercentUtility, P>
    ? `To define this property you need to set OnepercentUtility.${P} globally`
    : import("type-fest").Get<OnepercentUtility, P>;
export {};
